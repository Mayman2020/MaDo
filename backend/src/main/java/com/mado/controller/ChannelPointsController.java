...
public class ChannelPointsController {
    ...
    public ResponseEntity<ChannelPointReward> updateReward(
            @PathVariable String username,
            @PathVariable UUID rewardId,
            @RequestBody Map<String, Object> body,
            @AuthenticationPrincipal CustomUserDetails principal) {
        return ResponseEntity.ok(channelPointsService.updateReward(username, rewardId, body, principal.user()));
    }
    ...
    public ResponseEntity<Void> deleteReward(
            ...
            @PathVariable UUID rewardId,
            @AuthenticationPrincipal CustomUserDetails principal) {
        channelPointsService.deleteReward(username, rewardId, principal.user());
